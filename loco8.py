...
def getOrm_AT_x(self):
    ...
    for j in range(len(self.correctors_indexes)):
        ...
        s_pos = lindata['s_pos']
        closed_orbit[j] = lindata['closed_orbit']
        closed_orbitx[j] = lindata['closed_orbit'][:, 0]
        closed_orbity[j] = lindata['closed_orbit'][:, 1]

        #self.lattice[self.correctors_indexes[j]].KickAngle = [0, 0.00]
        cx.append(closed_orbitx[j])
        ...
        index = cx.index(closed_orbitx[j])
        # open file
        file = open("orm_x_CXY_"+ str(index) +".txt", "w")
