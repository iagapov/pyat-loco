...
def getOrm_AT_x(self):
    ...
    for j in range(len(self.indexes)):
        ...
        for i in indx:
            ...
            fh = open("C:/Users/musa/pyat-loco-1/fodo_loco/mydata/orm_x/orm_x_CXY_" + str(i[0]) + str(i[1]) + ".csv", 'w',  newline='')
            csv_writer = csv.writer(fh)

            # write one row with headers (using `writerow` without `s` at the end)
            csv_writer.writerow(["KickAngle", "closed_orbitx","closed_orbity"])
            fh.close()

            for k in self.indexes:
                    lindata0, tune, chrom, lindata = self.lattice.linopt(get_chrom=True, refpts=self.lattice[k].FamName)
                    s_pos = lindata['s_pos']
                    closed_orbit = lindata['closed_orbit']
                    closed_orbitx = lindata['closed_orbit'][:, 0]
                    closed_orbity = lindata['closed_orbit'][:, 1]
                    ...
                    str2 = closed_orbitx
                    str3 = closed_orbity
                    fh = open("C:/Users/musa/pyat-loco-1/fodo_loco/mydata/orm_X/orm_x_CXY_" + str(i[0]) + str(i[1]) + ".csv",
                          'a',  newline='')  # `a` for `append mode`
                    csv_writer = csv.writer(fh)
                # write row row with result (using `writerow` without `s` at the end)
                    csv_writer.writerow([str1], [str2], [str3])
            fh.close()
...
def getOrm_AT_y(self):
    ...
    for j in range(len(self.indexes)):
        ...
        for i in indx:
            ...
            fh = open("C:/Users/musa/pyat-loco-1/fodo_loco/mydata/orm_Y/orm_y_CXY_" + str(i[0]) + str(i[1]) + ".csv", 'w',  newline='')
            csv_writer = csv.writer(fh)
            ...
            for k in self.indexes:
                    ...
                    fh = open("C:/Users/musa/pyat-loco-1/fodo_loco/mydata/orm_Y/orm_y_CXY_" + str(i[0]) + str(i[1]) + ".csv", 'a',  newline=''
                          )  # `a` for `append mode`
                    csv_writer = csv.writer(fh)
                # write row row with result (using `writerow` without `s` at the end)
