...
def getOrm_AT_x(self):
    ...
    for j in range(len(self.correctors_indexes)):
        ...
        for i in range(len(indx)):
            file = open("C:/Users/musa/pyat-loco-1/fodo_loco/mydata/orm_x_CXY_" + str(indx[i]) + ".txt", "w")
            for l in range(len(self.correctors_indexes)):
               str1 = repr(self.lattice[self.correctors_indexes[l]].KickAngle)
               file.write(str1+ "\n")
            file.close()
