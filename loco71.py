...
def getOrm_AT_x(self):
    ...
    for j in range(len(self.indexes)):
        ...
        for i in indx:
            ...
            l =0
            while(l < len(self.indexes):

                 str1 = float(self.lattice[self.indexes[l]].KickAngle[0])
                 lindata0, tune, chrom, lindata = self.lattice.linopt(get_chrom=True, refpts=self.lattice[self.indexes[l]].FamName)
                 s_pos = lindata['s_pos']
                 closed_orbit = lindata['closed_orbit']
                 closed_orbitx = lindata['closed_orbit'][:, 0]
                 closed_orbity = lindata['closed_orbit'][:, 1]
