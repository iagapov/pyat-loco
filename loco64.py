...
class Loco:
    ...
    def prepareTwiss(self):
        ...
        if self.engine == 'at':
            ...
            self.opt = at.linopt(self.lattice, refpts=self.indexes, get_chrom=True)
            tune = self.opt[1]
            chrom = self.opt[2]
...
def getOrm_AT_x(self):
    ...
    for j in range(len(self.indexes)):
        ...
        for i in indx:
            file = open("C:/Users/musa/pyat-loco-1/fodo_loco/mydata/orm_X/orm_x_CXY_"+str(i[0])+str(i[1])+".txt", "w")
            for l in self.indexes:

                #self.opt = at.linopt(self.lattice, refpts=self.lattice[l].FamName, get_chrom=True)
                #tune = self.opt[1]
                #chrom = self.opt[2]
                #tws = self.opt[3]  # optics functions
                #str1 = float(self.opt[3].s_pos)
                #str1 = str(self.lattice[l].FamName)
                str1 = float(self.lattice[l].KickAngle[0])

                lindata0, tune, chrom, lindata = self.lattice.linopt(get_chrom=True, refpts=str(self.lattice[l].FamName))
                s_pos = lindata['s_pos']
                closed_orbit = lindata['closed_orbit']
                closed_orbitx = lindata['closed_orbit'][:, 0]
                closed_orbity = lindata['closed_orbit'][:, 1]
                ...
                #str3 = float(self.lattice[l].Length)

                #file.write(str1+"\n")
                #file.write(str1)
                #file.write("\n")
                print(str1, str2, str3, file=file)
            file.close()
