...
def getOrm_AT_x(self):
    ...
    for j in range(len(self.indexes_correctors)):
        ...
        correctors_s.append(s_pos)
        indx = np.argwhere(cx == closed_orbitx)
        elements_Strength_correctors = []
        correctors_s = []
        for l in self.indexes_correctors:
           elements_strength = self.lattice[l].KickAngle
           elements_Strength_correctors.append(elements_strength)
           lindata0, tune, chrom, lindata = self.lattice.linopt(get_chrom=True, refpts=self.lattice[l])
           s_pos = lindata['s_pos']
           correctors_s.append(s_pos)
        ...
        dict3 = {'s_pos': correctors_s, 'elements_strength': elements_Strength_correctors}
